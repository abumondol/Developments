...
public class PreProcess {
    ...
    public static String[] processAndGetTopicList(String[][][] data) throws Exception {
        ...
        for (i = 0; i < fileCount; i++) {
            ...
            for (j = 0; j < lineCount; j++) {
                topicString = addTopicsToList(topicList, data[i][j][MC.INDEX_POSITIVE_TOPICS]);
                data[i][j][MC.INDEX_POSITIVE_TOPICS] = topicString;
                topicString = addTopicsToList(topicList, data[i][j][MC.INDEX_NEGATIVE_TOPICS]);
                data[i][j][MC.INDEX_NEGATIVE_TOPICS] = topicString;
            }
        }
    ...
    public static int[][][] getAdviceGrid(String[][][] data, String[] topics) throws Exception {
        ...
        for (i = 0; i < appCount; i++) {
            ...
            for (j = 0; j < lineCount; j++) {
                ...
                topicStr = data[i][j][MC.INDEX_POSITIVE_TOPICS]; // for positive topic tags                                
                ...
                topicStr = data[i][j][MC.INDEX_NEGATIVE_TOPICS]; // for negative topic tags                                
                if (topicStr != null && topicStr.length() > 0) {
                    st = topicStr.split(";");
                    for (k = 0; k < st.length; k++) {
                        s = st[k].trim();
    ...
    public static void correctTopics(String[][][] data, String[][] topicList) throws Exception {
        ...
        int i, j, k;
        String newStr;
        topicList = MyUtils.toLowerCaseAndremoveSpace(topicList);

        for (i = 0; i < topicList.length; i++) {
            if (topicList.length <= 1) {
                System.out.println("Length problem correction topic list: " + i);
                return;
            }
            if (topicList[i].length >= 2 && topicList[i][1] != null && topicList[i][1].length() > 0) {
                newStr = "";
                for (j = 1; j < topicList[i].length; j++) {
                    newStr += topicList[i][j] + ";";
                }
                
                if (newStr.equals("$;")) {
                    replaceWordOccuranceInAdviceList(data, topicList[i][0] + ";", "", MC.INDEX_POSITIVE_TOPICS);
                    replaceWordOccuranceInAdviceList(data, topicList[i][0] + ";", "", MC.INDEX_NEGATIVE_TOPICS);
                } else {
                    replaceWordOccuranceInAdviceList(data, topicList[i][0] + ";", newStr + ";", MC.INDEX_POSITIVE_TOPICS);
                    replaceWordOccuranceInAdviceList(data, topicList[i][0] + ";", newStr + ";", MC.INDEX_NEGATIVE_TOPICS);
                }
