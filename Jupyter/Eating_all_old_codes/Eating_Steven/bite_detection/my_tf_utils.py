...
def one_axis_conv_net(x, name):
    ...
    with tf.name_scope(name):
        ...
        conv_2 = conv_layer(maxpool_1, size_in=8, size_out=8, ksize=[5,1], strides=[1,1], padding="VALID", name='conv_2')
        ...
        print('maxpool2_shape; ', sz)
        #flattened = tf.reshape(maxpool_2, shape=[-1, sz[1]*sz[2]*sz[3]], name="Flattened")
         
        return tf.reshape(maxpool_2, shape=[-1, sz[2], sz[3]], name="reorg")
...
def mc_conv_blstm(x, y, keep_prob):
