...
def process_data(filepath, sampling_rate = 16, time_problem_index=-1):
    ...
    d = mdpu.resample(d, sampling_rate)
...
for subj in range(7):
    ...
    for sess in range(2):
        ...
        if subj==0 and sess==0:
            dr = process_data(filepath, time_problem_index=14827)
        else:
            dr = process_data(filepath)
            
        print("====== Left Hand =====")
        filepath = path + "/0" + str(subj) + "/000" + str(sess) + "/watch_left_000" + str(sess) + ".csv"
        dl = process_data(filepath)
                
        print("====== Annotations =====")
        filepath= path + "/0" + str(subj) + "/000" + str(sess) + "/annot_events.csv";
        a = process_bite_annots(filepath)
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@", type(a))
...
mfu.write_file('data', 'lab_data_steven.pkl', data)


# In[10]:


print(data[0][0]["data_left"].shape)
