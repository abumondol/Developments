...
for subject in range(len(data)):
    for sess in range(len(data[subject])):               
        ...
        if np.sum(np.isnan(accel))>0 or np.sum(np.isnan(gyro))>0 or np.sum(np.isnan(grav))>0:
            print('\nSubject, Sess >> ', subject, sess)
            print(np.sum(np.isnan(accel)), np.sum(np.isnan(gyro)), np.sum(np.isnan(grav)))
        
        mx = max([accel[0,0], gyro[0,0], grav[0,0]])
        mn = min([accel[-1,0], gyro[-1,0], grav[-1,0]])
        
        #accel = accel[(accel[:,0]>=mx) & (accel[:,0]<=mn), :]
        #gyro = gyro[(gyro[:,0]>=mx) & (gyro[:,0]<=mn), :]
        #gyro = gyro[(gyro[:,0]>=mx) & (gyro[:,0]<=mn), :]        
        
        #print('  Start times: accel, gyro, grav, quat >> ', accel[0,0], gyro[0, 0], grav[0,0])
        #print('  Start times: accel, gyro, grav, quat >> ', accel[-1,0], gyro[-1, 0], grav[-1,0])
        #print('  Sample count: accel, gyro, grav, quat >> ', len(accel), len(gyro), len(grav))        
