...
source_folder = "C:/wamp/www/m2fed_watch/uploads/"
...
min_meal_process_interval = 30*1000 #milliseconds
min_ema_sent_interval = 30*60*1000 #milliseconds
meal_bite_max_distance = 60*1000 #milli second
min_meal_duration = 60*1000 #milliseconds
min_meal_bite_count = 3
min_wait_time_after_meal_to_decide = 3*60*1000 #milliseconds
...
def process_beacon_data(data, file_info):
    ...
    for s in data:
        ...
        if code == 1:
            ...
            battery_data += watch_id +"," + str(t) +"," + str(code) + "," + code2 + "," + battery_pct +"\n"            
            last_battery_time = t
            last_battery_pct = battery_pct
            battery_data_count += 1
            
        elif code == 1000:
            code = code + int(tokens[2])
            location_data += watch_id +",XX," + str(t) +"," + str(t) + "," + str(code) + "\n"            
    ...
    if len(location_data)>0:
        tm = datetime.fromtimestamp(last_location_time/1000.0)
        print("Location reading count:", location_data_count, ", Last reading at: ", tm)
        with open(dest_folders["location"]+to_date_str+"_location_data_"+ watch_id, "a") as file:
    ...
    if mp_count>0:
        ...
        for i in range(mp_count):
            if Ypr[i, 0]>=0.5:
                ...
                bite_str += watch_id + "," + str(file_info["upload_time"]) + "," + str(int(resdata[ix, 0])) + ",0," + str(Ypr[i, 0])+"\n"
...
def process_meals_ema(watch_id):
    ...
    if len(clusters) > 0:            
        for i in range(len(clusters)-1):
            c = clusters[i]
            meal_str += watch_id +","+ str(cur_time) +","+ str(int(c[0][0])) +","+ str(int(c[-1][0])) +",0,"+ str(len(c)) +"\n"


        c = clusters[-1]
        last_time_for_remove = c[0][0]-1
        print("Last cluster size: ", len(c))
        print("Cluster times:", milli_to_str(c[0][0]), " --- ", milli_to_str(c[-1][0]), " :: cur_time: ", milli_to_str(cur_time))
        if cur_time - c[-1][0] >= min_wait_time_after_meal_to_decide and c[-1][0] - c[0][0] >= min_meal_duration and len(c)>=min_meal_bite_count:
            ...
            if (cur_time - c[-1][0]) <= 30*60*1000:
                last_time_ema_sent[watch_id] = cur_time
                ema_str = watch_id + "," + str(last_time_ema_sent[watch_id]) + "," + str(last_time_ema_sent[watch_id]) +",0,0\n"
...
while True:    
    ...
    for f in file_list:
        ...
        try:
            ...
            shutil.move(source_folder+f, dest_folders["rawdata"] + to_date_str + "/" + f)
            
        except Exception as e:
            print("***** Error in file: "+f)            
            print(str(e))
            print(traceback.format_exc())
            shutil.move(source_folder+f, dest_folders["rawdata_error"]+f)            
