...
for sub = 2:2    
   ...
   % d1(:,2:4) = mysmooth(d1(:,2:4), 0.90);        
    ...
    annots2 = annots; %data(sub).annots_adjusted;
    ...
    for i = 1:annot_count
        ...
        ttl = strcat('sub:', num2str(sub), ', annot no:', num2str(i), ', time:', num2str(annots(i,1)),', index:', num2str(annots(i,3)), ', label:', num2str(label) );            
        x = d(:,1); % timestamps
        x = x - s;
        y = d(:,2:4);        
        plot(x, y);
