...
public class ControlledActivity2 extends Activity implements TextToSpeech.OnInitListener, RecognitionListener {
    ...
    long currentTime, initial_interval = 5 * 1000, interval = 5 * 1000;
    String displayStr;
    ...
    void logData(int state, String action, int resultCode, String data) {
        ...
        if (action.equals("stt")) {
            index++;
            Intent i = new Intent(context, WearIntentService.class);
            i.putExtra("data", date_time + "\n\nIndex: " + index + "\n\n" + data);
            startService(i);
        }
